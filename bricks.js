(() => {
  ...
  const stopGame = ({ showIdle = true } = {}) => {
    ...
    if (!showIdle) {
      ...
      showIcon();   // only show the center icon when desired
    }
  };

  launcher.onclick = () => {
    if (gameActive) {
      if (over) {                          // game ended, restart without showing icon
        stopGame({ showIdle: false });
        startGame();
      } else {
        stopGame({ showIdle: true });      // normal toggle off
      }
    } else {
      startGame();
    }
  ...
  // Show pulsing icon on idle start
  showIcon();
  window.addEventListener("keydown", e => {
    if (!gameActive && isSpaceKey(e)) {
      e.preventDefault();
      startGame();
    }
  });
